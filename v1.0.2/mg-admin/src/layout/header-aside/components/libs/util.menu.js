...
export function elSubmenu (h, menu) {
  ...
  else icon = <i slot="title" class="fa fa-cog"/>
  return <el-submenu
    key={ menu.path }
    index={ menu.path }>
    { icon }
    <span slot="title">{ menu.title || '未命名菜单' }</span>
    { menu.children.length>0 &&menu.children.map(child => createMenu.call(this, h, child)) }
  </el-submenu>
}
...
export function createMenu (h, menu) {
  if (menu.children === undefined || menu.children.length===0) return elMenuItem.call(this, h, menu)
