...
export default {
  ...
  components: {ViewMusicsInfo, GetMusicsInfo,},
  data() {
    return {
      show: {
        edit: false,
      ...
      fields: [
        ...
        {key: 'name', name: '音乐名称', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'author', name: '作者', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'url', name: '地址', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'cover', name: '封面', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'lrc', name: '歌词', show: true, align: "center", enableSort: false, fixed: false},
  ...
  methods: {
    ...
    getDataList() {
      try {
        ...
        dirMusicsList(this.query).then(res => {
          let datas = res.data || []
    ...
    goDelete(data) {
      this.$toast.confirm('此操作将永久删除该条数据, 是否继续?').then(() => {
        dirMusicsDelete(data.id).then(res => {
    ...
    //添加编辑
    goEdit(data) {
      this.$toast.showSmallModal(data && data.id ? '修改' : '增加', GetMusicsInfo, {updateData: data}, data => {
        if (data) {
          this.getDataList()
        }
      })
    ...
    //跳转到查看详情
    goView(data) {
      this.$toast.showSmallModal('查看', ViewMusicsInfo, {viewData: data || {}}, data => {
        // console.log(data)
      })
    }
  }
