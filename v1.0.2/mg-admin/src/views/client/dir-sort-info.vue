        ...
        <mk-search-form :model="query" v-if="permBtn.queryButton" @search="goPage(1)">
          <el-form-item class="inline-item" prop="sort_name">
            <el-input v-model.trim="query.params.sort_name" @clear="goPage(1)" @keyup.native.enter="goPage(1)"
                      clearable placeholder="输入分类名称搜索"
                      size="mini"></el-input>
          </el-form-item>
        </mk-search-form>
...
export default {
  name: 'dir-sort-info',
  components: {ViewSortInfo, GetSortInfo,},
  data() {
    return {
      show: {
        edit: false,
      ...
      fields: [
        ...
        {key: 'sort_name', name: '分类名称', show: true, align: "center", enableSort: false, fixed: false},
  ...
  methods: {
    ...
    //导出
    exportEcx() {
      exportExecl(this, this.fields, this.datas, '分类列表', new Date().toLocaleDateString() + '导出的分类列表')
    },
    //获取窗口高度
    getWinHeight() {
      let that = this
    ...
    goDelete(data) {
      this.$toast.confirm('此操作将永久删除该分类, 是否继续?').then(() => {
...
//添加编辑
    goEdit(data) {
      this.$toast.showMiniModal(data && data.id ? '修改分类' : '增加分类', GetSortInfo, {updateData: data}, data => {
        if (data) {
          this.getDataList()
        }
      })
...
//跳转到查看详情
    goView(data) {
      this.$toast.showMiniModal('查看分类', ViewSortInfo, {viewData: data || {}}, data => {
        // console.log(data)
      })
    }
  }
