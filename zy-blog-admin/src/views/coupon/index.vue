...
<template>
  <div class="app-table">
    <div class="app-table-query" v-if="listButton">
      <span style=" color:#23262b; font-size:13px; margin-left:17px">广告ID:</span>
      <el-select v-model="queryList.params.couponId" placeholder="请选择" size="mini">
        <el-option
    ...
    methods: {
      getPerms() {
        ...
        if (perms.includes('*')) {
          ...
          if (perms.includes('POST /admin/couponConsumeList')) {
            this.listButton = true;
          }
      ...
      getList() {
        ...
        this.loading = true
        this.request('api_admin_coupon_list', this.queryList).then((res) => {
          this.tableData = res;
          this.loading = false
        }).catch((error) => {
          this.loading = false
      ...
      doExcelExport() {
        ...
        this.loading = true
        this.request('api_admin_coupon_export', this.queryList).then((res) => {
          this.loading = false
        }).catch((error) => {
          this.loading = false
        });
