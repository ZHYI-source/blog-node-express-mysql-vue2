  ...
  <section class="data-list-box">
    <section v-if="!show.edit">
      ...
      >
        ...
        <el-form-item class="inline-item" prop="aboutTitle">
          <el-input v-model.trim="query.params.aboutTitle" clearable placeholder="输入关于标题搜索" size="mini"
                    @clear="goPage(1)" @keyup.enter.native="goPage(1)"
          ></el-input>
        </el-form-item>
      </lk-search-form>
    ...
    <GetWebAbout v-if="show.edit" :updateData="updateData" @close="closeEdit"/>
  </section>
</template>
...
import GetWebAbout from "@/views/blog/get-web-about";
import ViewWebAbout from "@/views/blog/view-web-about";
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {key: 'aboutTitle', name: '关于标题', show: true, width: '', enableSort: false, align: "center", fixed: false},
        // {key: 'aboutContent', name: '关于内容', show: true, width: '', enableSort: false, align: "center", fixed: false},
  ...
  methods: {
    ...
    getDataList() {
      try {
        ...
        this.request('api_blog_web_about_list', this.query).then(res => {
    ...
    goDelete(data) {
      ...
      this.toast.confirmDelete().then(() => {
        this.request('api_blog_web_about_delete', p).then(res => {
          this.$message({
            message: '删除成功！',
            type: 'success'
          });
