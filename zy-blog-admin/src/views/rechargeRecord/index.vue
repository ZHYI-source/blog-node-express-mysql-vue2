    ...
    methods: {
      getPerms() {
        ...
        if (perms.includes('*')) {
          ...
          if (perms.includes('POST /admin/invoice/depositList')) {
            this.listButton = true;
          }
          if (perms.includes('POST /admin/invoice/depositExport')) {
            this.exportButton = true;
          }
        }
      },
      ...
      getList() {
        ...
        this.loading = true
        this.request('api_admin_invoice_depositList', this.queryList).then((res) => {
          this.tableData = res;
          this.loading = false
        }).catch((error) => {
          console.log(error)
      ...
      handleExcelExport() {
        ...
        this.loading = true
        this.queryList.current = this.tableData.current
        this.request('api_admin_invoice_depositExport', this.queryList).then((res) => {
          this.loading = false
        }).catch((error) => {
          this.loading = false
        });
