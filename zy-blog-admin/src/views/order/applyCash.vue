...
<template>
  <div class="app-table">
    <div class="app-table-query" v-if="listButton">
      ...
      <el-select size="mini" v-model="query.applyState" placeholder="提现状态">
    ...
    <div class="app-table-contain">
      <div style="height: 100%;" ref="ztable">
        <el-table
          ...
          <el-table-column prop="userTel" label="手机号" width="200" align="center">
          </el-table-column>
          <el-table-column prop="account" label="银行卡号" width="200" align="center">
          </el-table-column>
          <el-table-column width="200" prop="applyTime" label="申请时间" align="center">
            <template slot-scope="scope">
              {{scope.row.applyTime|fmtTime}}
            </template>
          </el-table-column>
          <el-table-column width="200" prop="feeStr" label="提现金额" align="center">
          </el-table-column>
          <el-table-column prop="applyStateStr" label="提现状态" align="center">
          </el-table-column>
          <el-table-column label="操作" align="center" fixed="right" width="180">
            <template slot-scope="scope">
              <el-button-group>
                <el-button v-if="scope.row.applyState === 0" size="mini" type="primary" icon="el-icon-document"
                           @click="handleDeal(scope.row)">处理
                </el-button>
              </el-button-group>
            </template>
          </el-table-column>
    ...
    <el-dialog title="提示" :visible.sync="dealForm">
      <div style="margin:15px 10px 20px 20px">提现状态将变为已完结，是否确定处理?</div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dealForm = false">
          取消
        </el-button>
    ...
    methods: {
      getPerms() {
        ...
        if (perms.includes('*')) {
          ...
          if (perms.includes('POST /admin/exportAppCash')) {
            this.exportButton = true;
          }
          if (perms.includes('POST /admin/appCashLst')) {
            this.listButton = true;
          }
        }
      },
      handleDeal(row) {
        this.dealForm = true
        this.queryDeal.keyId = row.keyId
      },
      dealData() {
        this.request('api_admin_cashOutDeal', this.queryDeal).then((res) => {
          this.doQuery()
          this.dealForm = false
          this.$notify.success({
            title: '成功',
      ...
      doExcelExport() {
        ...
        this.request('api_admin_exportAppCash', p).then((res) => {
          // this.tableData = res;
        }).catch((error) => {
        });
      ...
      queryData(val) {
        let p = val;
        this.request('api_admin_appCashLst', p).then((res) => {
          this.tableData = res;
        }).catch((error) => {
        });
      },
