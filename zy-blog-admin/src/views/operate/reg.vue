<style lang="scss" src="@css/app.scss"></style>
<template>
  <div>
    <div class="app-form-class">
      <el-form ref="form" :model="formVal" :rules="rules" label-width="80px" size="mini" @submit.native.prevent
               :hide-required-asterisk="ctl.view">
        <el-row :gutter="20" type="flex" justify="space-between">
          <el-col :span="11">
            <el-form-item label="姓名" :required="true" prop="accountName">
              <el-input v-model="formVal.accountName" :disabled="true">
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="手机号" :required="true" prop="mobileNo">
              <el-input v-model="formVal.mobileNo"></el-input>
            </el-form-item>
          </el-col>
        </el-row>

        <el-row :gutter="20" type="flex" justify="space-between">
          <el-col :span="11">
            <el-form-item label="身份证号" :required="true" prop="idCard">
              <el-input v-model="formVal.idCard" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="银行卡号" :required="true" prop="account">
              <el-input v-model="formVal.account"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    </div>
    <el-row type="flex" justify="center" style="padding: 16px">
      <el-col :span="12" style="display:flex;justify-content: center">
        <el-button type="primary" @click="addOrUdt()" v-if="!ctl.view" :loading="ctl.loading">保存</el-button>
        <el-button @click="appClose()">关闭</el-button>
      </el-col>
    </el-row>

  </div>

</template>

<script>
  import tool from '@libs/tool';

  export default {
    components: {},
    props: {},
    data() {
      return {
        ctl: {
          view: false,
          loading: false
        },
        formVal: {},
        nh: {},
        dateTime: [],
        company: [],
        rules: {
          userTel: [
            {required: true, message: '请输入手机号'}
            // {pattern: /^[a-zA-Z0-9]+$/, message: '车牌格式错误'}
          ],
          idCard: [
            {required: true, message: '请输入身份证号'}
          ],
          account: [
            {required: true, message: '请输入农行卡号'}
          ],
          accountName: [
            {required: true, message: '请输入姓名'}
          ]
          // vehicleCarPubTime: [
          //     {required: true, message: '请选择发证时间'},
          //     {required: true, message: '请选择发证时间'},
          // ],
          // companyId: [
          //     {required: true, message: '请选择所属公司'},
          //     {required: true, message: '请选择所属公司'},
          // ],
          // vehicleCarType: [
          //     {required: true, message: '请填写车型'},
          // ]
        }
      };
    },
    computed: {},
    methods: {
      appClose() {
        this.$emit('appClose');
      },
      addOrUdt() {
        this.$refs['form'].validate((valid) => {
          if (!valid) {
            return false;
          }
          let p = tool.cloneObj(this.formVal);
          this.ctl.loading = true;
          this.request('api_admin_pay_reg', p).then((res) => {
            this.ctl.loading = false;
            this.$notify.success({
              title: '成功',
              message: '进件成功'
            })
            this.$emit('reload');
            this.$emit('appClose');
          }).catch((error) => {
            this.ctl.loading = false;
            window.Bus.$emit('yolo_pop_message_emit', error);
          });


        });
      },
      open(val, read) {
        this.formVal = {
          idCard: val.idCard,
          mobileNo: val.userTel,
          account: val.account,
          accountName: val.userRealName
        };
        // tool.cloneObj(val);
        this.ctl.view = read;
      },
      queryNHTotal(shopNo) {

        if (this.strEmpty(shopNo)) {
          return;
        }

        let p = {
          shopNo: shopNo
        };
        this.request('api_admin_pay_querySubmerchatFee', p).then((res) => {
          this.nh = res;
        }).catch((error) => {
          window.Bus.$emit('yolo_pop_message_emit', error);
        });

      }

    },
    mounted() {

    }

  }
</script>

<style scoped>

</style>
