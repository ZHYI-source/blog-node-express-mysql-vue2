<template>
  <section>
    <lk-view-header @back="$emit('close')" :content=headTitle />
    <section class="driver-view-box">
      ...
      <section class="driver-info">
        ...
        <lk-view-row>
          <lk-view-item title="所属机构">{{baseData.orgName}}</lk-view-item>
        </lk-view-row>
        <lk-view-row>
...
export default {
  ...
  created() {
    this.baseDataTemp=this.baseData
    this.loadData(this.dataId);
  },
  mounted(){
    this.loadData(this.dataId);
    this.headTitle = this.baseData.driverName + '的'+ (new Date(this.baseData.time).getMonth()+1 ) + '月报表详情'
  },
  ...
  methods: {
    ...
    loadData(id) {
      let that =this
      this.request('api_admin_comment_reportPerMonthDetail', {idCard:id}).then(res => {
        this.form = res || {};
