<template>
  <section>
    <lk-view-header @back="$emit('close')" :content="form.orgName + ' 月报详情'"/>
    <section class="driver-view-box">
      ...
      <section class="driver-info">
        <lk-view-row>
          <lk-view-item title="机构名称" label-width="90">{{form.orgName}}</lk-view-item>
        </lk-view-row>
        <lk-view-row>
          <lk-view-item title="企业车辆" label-width="90">{{form.countCar + ' 台'}}</lk-view-item>
        </lk-view-row>
        <lk-view-row>
          <lk-view-item title="该月评价次数" label-width="90">{{form.countComment + ' 次'}}</lk-view-item>
        </lk-view-row>
        <lk-view-row>
          <lk-view-item title="该月评价得分" label-width="90">
            {{form.score}} 分
          </lk-view-item>
        </lk-view-row>
      </section>
...
export default {
  ...
  methods: {
    ...
    loadData(data) {
      let that =this
      this.request('api_admin_comment_orgReportDetail', data).then(res => {
        this.form = res || {};
