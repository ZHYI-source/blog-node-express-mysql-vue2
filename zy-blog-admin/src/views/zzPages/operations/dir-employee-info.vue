    ...
    <section>
      ...
      <lk-el-table :key="updataKey" :datas="datas" stripe :height="tableHeight" :loadingText="loading.text" :loading="loading.list">
        <template v-for="field in fields" v-if="field.show">
          ...
          <el-table-column v-else-if="field.key=='headImg'" :align="field.align" :label="field.name"
                           :width="field.width" :fixed="field.fixed"
          >
            <template slot-scope="scope">
              <el-image
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {key: 'workerName', name: '用户姓名', show: true, width: '180', enableSort: false, align: "center", fixed: false},
        {key: 'phone', name: '用户手机号', show: true, width: '180', enableSort: false, align: "center", fixed: false},
        {key: 'account', name: '银行卡号', show: true, width: '180', enableSort: false, align: "center", fixed: false},
        {key: 'bankName', name: '银行名称', show: true, width: '180', enableSort: false, align: "center", fixed: false},
        {key: 'headImg', name: '身份证照片', show: true, width: '150', enableSort: false, align: "center", fixed: false},
        {key: 'address', name: '居住地址', show: true, width: '', enableSort: false, align: "center", fixed: false},
        {key: 'nativePlace', name: '用户籍贯', show: true, width: '', enableSort: false, align: "center", fixed: false},
        {key: 'isReal', name: '是否实名', show: true, width: '', enableSort: false, align: "center", fixed: false},
        {key: 'isSign', name: '是否签约', show: true, width: '', enableSort: false, align: "center", fixed: false},
        {key: 'insertTimestamp', name: '插入时间', show: true, width: '180', enableSort: false, align: "center", fixed: false},
        {key: 'toolButton', name: '操作', show: true, width: '150', enableSort: false, align: "center", fixed: 'right'},
      ]
    }
  },
  created() {
  ...
  methods: {
    ...
    goEdit(data) {
      let datas = data ? data : {};
      this.toast.showSmallModal(datas.idCard?'修改用户':'添加用户', GetEmployeeInfo, {datas}, res => {
        this.getDataList()
      })
    },
    //关闭编辑
    ...
    goView(data) {
      let datas = data ? data : {};
      this.toast.showSmallModal('用户详情', ViewEmployeeInfo, {datas})
    },
    //下载用户模板
    getExample() {
      let params = {};
